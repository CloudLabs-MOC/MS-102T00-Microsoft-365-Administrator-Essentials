# Lab 04 - Exercise 4: PIM Teammate Approval Request
...
13. In the **Intune Administrator | Assignments** window, select **+ Add assignments** on the menu bar.
    ...
    ![](../Images/p11t1p2-july26.png)

14. In the **Add assignments** window, the **Membership** tab is displayed by default. Under **Select member(s)**, select **No member selected (1)**.

15. In the **Select a member** pane that appears on the right, enter **PIM-Intune-Administrators (2)** in the **Search** field. This will display the list of eligible users and groups whose name starts with **PIM**. Select the **PIM-Intune-Administrators (3)** group that appears, and then select the **Select (4)** button.

    ![](../Images/p11t1p3-july26.png)

16. In the **Add assignments** window, select **Next >** (this does the same thing as selecting the **Setting** tab).

17. In the **Add assignments** window, under the **Setting** tab, verify the **Assignment type** option is set to **Eligible (1)**. Also verify the **Permanently eligible (2)** check box is selected (if not, then do so now), and then select **Assign (3)**.

    ![](../Images/p11t1p4-july26.png)
...
2. In your browser, you should still have the **Microsoft Entra admin center** open from the prior task. In the left-hand navigation pane, under the **Id Governance** section, select **Privileged Identity Management (1)**.

3. In the **Privileged Identity Management | Quick start** window, in the middle pane under the **Manage** section, select **Microsoft Entra roles (2)**.

   ![](../Images/p11t1p1-july26.png)

4. In the **OTU WA MOC | Quick start** window, in the middle pane under the **Manage** section, select **Settings (1)**.

5. In the **OTU WA MOC | Settings** window, select the **Intune Administrator (2)** role.

   ![](../Images/p11t1p7-july26.png)
...
12. In the **Edit role setting - Intune Administrator** window, you're currently in the **Activation** tab. Select the **Next: Assignment (5)** tab that appears next to it.
    ...
    ![](../Images/p11t1p8-july26.png)

13. Verify the **Require justification on active assignment (1)** check box is selected (if not, select it now).

14. In the **Edit role setting - Intune Administrator** window, select the **Next: Notification (2)**.

    ![](../Images/ms102-p12t2p3.png)

15. On the **Notification** tab, under the **Send notifications when eligible members activate this role (1)** section, Holly wants to be notified when Alex or Joni approve this role. Therefore:
    ...
    - Verify the **Request to approve an activation** check box is selected.
      ...
      > **Note:** Click **OK** to confirm disabling approval email notifications for this role.

      ![](../Images/p10t1p3-july26.png)

16. At the bottom of the **Edit role setting - Intune Administrator** window, select **Update (2)**.
...
4.  In the **Microsoft Azure** portal, in the middle of the screen is the section of **Azure services**. This section displays a row of Azure services and their associated icons. At the end of the row, select **More services** (with the forward arrow icon). This opens the **All services** window.

    ![](../Images/p10t1p4-july26.png)

5.  In the **All services** window, enter **priv** in the **Filter services** search box at the top of the page. In the list of search results, select **Microsoft Entra Privileged Identity Management**.

    ![](../Images/p10t1p5-july26.png)

6.  In the **Privileged Identity Management | Quick start** window, in the **Tasks** section in the left-hand navigation pane, select **My Roles**.

    ![](../Images/ms102-p12t2p5.png)

7.  In the **My roles | Microsoft Entra roles** window, the **Eligible assignments** tab is displayed by default. Remember, in the prior task Holly assigned Joni and Alex as members of the **PIM-Intune-Administrators** group, which Holly later assigned as an eligible group for the Intune Administrator role. As such, this role appears in the list of **Eligible assignments** for Joni. Under the **Action** column for the Intune Administrator role, select **Activate**.

    ![](../Images/ms102-p12t2p6.png)

8.  In the **Activate - Intune Administrator** pane that appears, enter **Device management support requests from various users that require resolution** in the **Reason (1)** field, and then select the **Activate (2)** button at the bottom of the pane.

    ![](../Images/ms102-p12t2p7.png)

9.  On the **My roles | Microsoft Entra roles** window, the **Eligible assignments** tab is displayed on the menu bar. Select the **Active assignments** tab that appears next to it. Note that no roles appear.
...
10. Leave the InPrivate browser session open for the next task.
