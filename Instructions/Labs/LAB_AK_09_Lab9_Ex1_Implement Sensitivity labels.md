...
1. On the **Microsoft Purview** portal, in the left-hand navigation pane, under **Solutions (1)**, select **Information protection (2)** and then select **Sensitivity labels (3)**.

   ![](../Images/p23t1p2-july26.png)

   ![](../Images/p23t1p3-july26.png)
...
1. On the **Sensitivity labels** page, select **+ Create a label**. This starts the **New sensitivity label** wizard.

   ![](../Images/p23t1p4-july26.png)

1. In the **New sensitivity label** wizard, on the **Name and tooltip** page, enter the following information:
   - Name: **PII** **(1)**
   - Display name: **PII** **(2)**

   - Description for users: **Documents, files, and emails with PII** **(3)**

   - Description for admins: **Documents, files, and emails with PII** **(4)**

   - Label color: select one of the colors that you want to assign to your sensitivity labels **(5)**

1. Select **Next (6)**.

   ![](../Images/p23t1p5-july26.png)

1. On the **Define the scope for this label** page, verify that the **Files & other data assets**, **Emails**, and **Meetings** **(1)** check boxes contain a tick mark. These check boxes allow you to define where this sensitivity label will be used so that you can configure the applicable protection settings. Select **Next (2)**.

   ![](../Images/p23t1p6-july26.png)
...
1. On the **Choose protection settings for the types of items you selected** page, you can begin the process of using encryption settings to restrict access to content that the label will be applied to. When a document, email, or meeting invitation is encrypted, access to content is restricted, so that it: <br/>
   ...
   Holly wants to configure encryption settings, so on the **Choose protection settings for the types of items you selected** page, select the **Control access (1)** check box. Also select the **Apply content marking (2)** check box so that you can later configure header, footer, and watermark settings. Select **Next (3)**.

   ![](../Images/p23t1p7-july26.png)
...
1. As such, select the **Remove access control settings if already applied to items (1)** option and then select **Next (2)**.

   ![](../Images/p23t1p8-july26.png)

1. Because you selected the **Apply content marking** option on the earlier page, the **Content marking** page now appears. Set the **Content marking (1)** toggle switch to **On**. This displays three options that enable you to customize how you want to mark files and emails.

   ![](../Images/p23t1p9-july26.png)

1. Select all three check boxes. Under each setting, select **Customize text**. This opens a pane to customize that particular setting. Enter the following information in the **Customize (1)** pane for each option (select **Save** after entering the settings for each option):
   - **Add a watermark**
     - Watermark text: **Sensitive - Do Not Share (2)** (Hint: after entering this value, copy it so that you can paste it in the other two text settings)
     - Font size: **`25` (3)**
     - Font color: **Blue (4)**
     - Text layout: **Diagonal (5)**
     - Select **Save (6)**

       ![](../Images/p23t1p10-july26.png)
...
1. On the **Content Marking** page, select **Next**.
   ...
   ![](../Images/p23t1p11-july26.png)

1. On the **Auto-labeling for files and emails** page, set the **Auto-labeling for files and emails** toggle switch to **On (1)**. This enables a series of options that you will update in the next steps.

1. Under **Detect content that matches these conditions**, select **+ Add a condition (2)** and then select **Content contains (3)**.

   ![](../Images/p23t1p12-july26.png)

1. In the **Content contains** window, select the **Add (1)** drop-down arrow and then select **Sensitive info types (2)**.

   ![](../Images/p23t1p13-july26.png)

1. In the **Sensitive info types** window, hover your mouse to the left of the **Name (1)** column heading. Select the check box that appears, which automatically selects all the sensitive information types. Select **Add (2)**, which will add all these sensitive information types to your label.

   ![](../Images/p23t1p14-july26.png)
...
1. On the **Auto-labeling for files and emails** page, all of the sensitive information types that you selected will be displayed. Scroll to the bottom on the window and update the following settings:
   - When content matches these conditions: select **Automatically apply the Label (1)**

   - Display this message to users when the label is applied: enter **`Sensitive content has been detected and will be encrypted.`** **(2)**

1. Select **Next (3)**.

   ![](../Images/p23t1p15-july26.png)

1. On the **Define protection settings for groups and sites** page, leave all check boxes blank and select **Next**.

   ![](../Images/p23t1p16-july26.png)
...
1. Select the **Publish labels** option that appears in the menu bar above the list of labels. This initiates a **Create policy** wizard.
   ...
   ![](../Images/p23t1p22-july26.png)

1. In the **Create policy** wizard, on the **Choose sensitivity labels to publish** page, under **Sensitivity labels to publish**, **PII** label is already selected, select **Next** for **Assign admin units**.

   ![](../Images/p23t1p23-july26.png)

1. On **Assign admin units** select **Next**.

   ![](../Images/p23t1p24-july26.png)

1. On the **Publish to users and groups** page, the **Exchange email** location is already selected by default (if not, select it now), with a scope of **All users & groups**. Select **Next**.

   ![](../Images/p23t1p25-july26.png)

1. On the **Policy settings** page, select the **Users must provide a justification to remove a label or lower its classification (1)** check box, and then select **Next (2)**.

   ![](../Images/p23t1p26-july26.png)

1. On the **Apply a default label to documents** page, select **PII** in the drop-down menu that appears, and then select **Next**.

   ![](../Images/p23t1p27-july26.png)

1. On the **Apply a default label to emails** page, select **PII (1)** in the drop-down menu that appears, and then select **Next (2)**.

   ![](../Images/p23t1p28-july26.png)

1. On the **Apply a default label to meetings and calendar events** page, select **None (1)** in the drop-down menu that appears, and then select **Next (2)**.

   ![](../Images/p23t1p29.png)

1. On the **Apply a default label to Fabric and Power BI content** page, select **PII (1)** in the drop-down menu that appears, and then select **Next (2)**.

   ![](../Images/p23t1p30-july26.png)

1. On the **Name your policy** page, enter **PII Policy (1)** in the **Name** field, and then enter (or copy and paste) the following description for this sensitivity label policy: `The purpose of this policy is to detect sensitive information such as ABA bank routing numbers and US social security numbers in emails and documents, and to encrypt this information when it's discovered. The user must provide an explanation for removing the classification label.` **(2)** Select **Next (3)**.

   ![](../Images/p23t1p31-july26.png)

1. On the **Review and finish** page, review the information you entered. If anything needs to be corrected, select the corresponding **Edit** option and make the necessary corrections. When all information is correct, select **Submit**.

   ![](../Images/p23t1p32-july26.png)
...
1. On the **M365 Copilot** tab, select **Apps (1)** from the top of left navigation pane, select the **Word (2)** icon. This will navigate you to the Microsoft Word Online.

   ![](../Images/p23t1p33-july26.png)
...
1. Because you enabled Sensitivity labels at the start of this exercise, and since the label you created by running the script on day 1 of the course has fully propagated through Microsoft 365, Word should display a **Sensitivity** group on the ribbon at the top of the page. Select the down arrow in the **Sensitivity** group. In the drop-down menu that appears, it should display the **PII** label the script created and published. Since the **PII** label is enabled for this document, a check mark is displayed next to **PII**.
   ...
   ![](../Images/p23t1p36-july26.png)

   > **Note:** If the **Sensitivity** option isn't visible, wait a few minutes and refresh the browser.
...
1. In the **Sensitivity** group in the Word ribbon, select the down arrow. In the drop-down menu that appears, note that while **PII** is displayed, it is no longer selected. This indicates the **PII** sensitivity label is no longer being applied to this document.

   ![](../Images/p23t1p37-july26.png)

1. To re-apply the sensitivity label to the document, select **PII** in the drop-down menu. Once again select the drop-down arrow in the **Sensitivity** group. The drop-down menu that appears should display the **PII** label, and select it.
...
1. You will now save the document. On the title bar, to the right of Word, select **Document**. In the drop-down menu that appears, confirm the file **Location** says **Holly Dickson > Documents**.

   ![](../Images/p23t1p39-july26.png)
...
1. On the right-side of the menu bar, select the **Share (1)** button. In the drop-down menu that appears, select **Share (2)**.

   ![](../Images/p23t1p40-july26.png)
...
6. In the right-hand pane, enter the following information in the message form:
   - To: Enter **Joni** and then select **Joni Sherman (1)** from the user list.

   - CC: Enter your own personal email address (do NOT enter Holly's email address; instead, enter your own personal email address) **(2)**

   - Add a subject: **Protected Document Test (3)**

   - Body of the message: enter **If you can open the protected and restricted document attached to this email, then try to change it. (4)**

7. In the body of the message, under the text you added in the previous step, paste the link copied to your clipboard from the prior task. A link for the file named **ProtectedDocument1.docx (4)** should appear.

8. Select **Send (5)**.

   ![](../Images/p23t1p42-july26.png)
...
18. In the **Your privacy option** dialog box that appears, select **Close**. Review the document and then close the document window.
...
22. When a **Sign in** window appears, it's because the file is RMS protected and no AIP unified labeling client is installed on LON-CL2. In this case, you need to use the native RMS features of Word Microsoft Apps and register this installation to Joni’s account.

    ![](../Images/p23t1p44-july26.png)
...
24. On the **Sign in to all apps and websites on this device?** pop-up window, select **No, this app only**.

    ![](../Images/p23t1p45-july26.png)

25. The file should open in MS Word, since you assigned Joni with Read-only permission. Review the three notification bars that appear above the document.
    ...
    ![](../Images/p23t1p46-july26.png)

26. Close the Word Tab.

27. You will now test what happens when you attempt to open the document that was sent to your personal email address. Use your phone or classroom PC to access your personal email address. Open the email that you (in the role of Holly) just sent to your personal email address, and then attempt to open the attached file.

28. You should receive a message indicating that you are not signed into Office with an account that has permission to access the document. You can optionally sign in with an account that has permission to access the file, or request access from the **Holly**'s account account, or Cancel out of the operation. Select **Cancel**. Since only Joni was assigned permission to read the document, you just verified that Azure Information Protection protected the document based on the PII policy parameters that you configured.
